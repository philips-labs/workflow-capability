...
class Cdr:
    ...
    def create_observation_quantity(self, category, code, patient, value, valueQuantity_unit,valueQuantity_code):
        jsoncp = {
    ...
    def create_observation_string(self, category, code, patient, value):
        jsoncp = {
            'resourceType': 'Observation',
            'status': 'final',
            'category': [ {
    ...
    def create_patient(self, patient_props: dict):
        ...
        response = requests.post(self.base_url + patient_url, data=str(patient.json()), headers=self.header)
        return json.loads(response.text) if response else None
